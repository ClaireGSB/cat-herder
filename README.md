...
1.  It first checks that your repository has no uncommitted changes. If any are found, the task will abort with an error message asking you to commit or stash your changes first.
2.  It checks out the `main` branch and pulls the latest changes from origin to ensure a fresh starting point.
3.  It creates a unique, dedicated branch from the updated `main` branch for the task (e.g., `claude/my-new-feature`).
