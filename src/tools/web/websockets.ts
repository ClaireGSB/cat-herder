...
export function setupWebSockets(server: Server, stateDir: string, logsDir: string): void {
  ...
  const clientWatchedLogFiles = new Map<WebSocket, { [logFile: string]: number }>(); // <--- ADD THIS LINE

  wss.on('connection', (ws: WebSocket) => {
      console.log('[WebSockets.ts] WebSocket client connected.'); // <--- UPDATED LOG
          ...
          try {
              ...
              if (data.type === 'watch_log') {
                  ...
                  if (!taskId || !logFile || typeof taskId !== "string" || typeof logFile !== "string") {
                      ...
                      console.warn(`[WebSockets.ts] Received invalid watch_log request: taskId=${taskId}, logFile=${logFile}`); // <--- ADDED LOG
                      return;
                  }
                  ...
                  if (!resolvedPath.startsWith(resolvedLogsDir + path.sep)) { // Use path.sep for robust check
                      ...
                      console.warn(`[WebSockets.ts] Access denied for log file path: ${filePath}`); // <--- ADDED LOG
                      return;
                  }
                  console.log(`[WebSockets.ts] Client requested to watch log: ${filePath}`); // <--- ADDED LOG
                  ...
                  if (existingWatcher) {
                      ...
                      console.log(`[WebSockets.ts] Closed previous watcher for client's active log.`); // <--- ADDED LOG
                  }
                  ...
                  if (fs.existsSync(filePath)) {
                      ...
                      console.log(`[WebSockets.ts] Sent initial 'log_content' for ${logFile}. Initial size: ${initialContent.length}`); // <--- ADDED LOG
                      ...
                      watcher.on('change', (watchedPath) => {
                          try {
                              ...
                              if (newContent.length > lastSize) {
                                  ...
                                  console.log(`[WebSockets.ts] Sent 'log_update' for ${logFile}. New chunk size: ${chunk.length}, New total size: ${newContent.length}`); // <--- ADDED LOG
                                  ...
                                  console.log(`[WebSockets.ts] Log file ${logFile} truncated, re-sent full content. New size: ${newContent.length}`); // <--- ADDED LOG
                              }
                          } catch (error) {
                              console.error(`[WebSockets.ts] Error reading file during watch for ${filePath}:`, error); // <--- UPDATED LOG
                      ...
                      watcher.on('error', (error) => {
                          console.error(`[WebSockets.ts] File watcher error for ${filePath}:`, error); // <--- UPDATED LOG
                      ...
                      console.warn(`[WebSockets.ts] Log file not found at ${filePath} when requested.`); // <--- ADDED LOG
                  }
              }
          } catch (e) {
      ...
      ws.on('close', () => {
          console.log('[WebSockets.ts] WebSocket client disconnected.'); // <--- UPDATED LOG
          if (clientWatchers.has(ws)) {
              clientWatchers.get(ws)?.close();
              clientWatchers.delete(ws);
              clientWatchedLogFiles.delete(ws); // <--- ADD THIS LINE (already in previous plan)
          }
      });

      ws.on('error', (error) => {
          console.error('[WebSockets.ts] WebSocket error:', error); // <--- UPDATED LOG
          if (clientWatchers.has(ws)) {
              clientWatchers.get(ws)?.close();
              clientWatchers.delete(ws);
              clientWatchedLogFiles.delete(ws); // <--- ADD THIS LINE (already in previous plan)
  ...
  const handleStateChange = (filePath: string) => {
      ...
      console.log(`[WebSockets.ts] State file change detected: ${path.basename(filePath)}`); // <--- ADDED LOG
      ...
      const attemptRead = () => {
          ...
          try {
              ...
              if (!dataToSend) {
                  console.error(`[WebSockets.ts] Could not get details for task ID ${stateData.taskId}. Aborting broadcast.`); // <--- UPDATED LOG
                  return;
              }
              ...
              console.log(`[WebSockets.ts] Broadcasted ${messageType} for ${path.basename(filePath)} (Attempt ${attempts})`); // <--- ADDED LOG

          } catch (e: any) {
              if (e instanceof SyntaxError && attempts < maxAttempts) {
                  console.warn(`[WebSockets.ts] Failed to parse ${path.basename(filePath)} (Attempt ${attempts}/${maxAttempts}), retrying shortly...`); // <--- UPDATED LOG
                  setTimeout(attemptRead, 75);
              } else {
                  console.error(`[WebSockets.ts] Failed to process state change for ${filePath}:`, e); // <--- UPDATED LOG
              }
          }
      };
      attemptRead();
